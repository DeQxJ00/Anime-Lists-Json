...
返回该 AniDB ID 对应的完整 `anime` 信息，字段来自 XML 的属性和子节点。
...
Action 会先比较上游 XML 的 SHA256。没有变化时会直接结束，不运行生成脚本；有变化时才重新生成：
...
3. 访问 `https://<username>.github.io/<repo>/api/anidb/1.json`。
